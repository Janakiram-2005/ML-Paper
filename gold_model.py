...
TARGET = "price_gold"
PLOT_NAME = "Dig-4.png"
MODEL_FILE = "gold_lstm_model.keras"
...
def build_features(merged_df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["gold_sma_5"] = df["price_gold"].rolling(window=5).mean()
    df["gold_sma_20"] = df["price_gold"].rolling(window=20).mean()
    df["gold_sma_50"] = df["price_gold"].rolling(window=50).mean()

    for lag in LAGS:
        df[f"price_gold_lag_{lag}"] = df["price_gold"].shift(lag)
...
def create_lstm_sequences(df_ml: pd.DataFrame):
    ...
    for lag in LAGS:
        selected_features.append(f"price_gold_lag_{lag}")
...
def plot_predictions(test_dates, y_true, y_pred, output_path: Path):
    ...
    plt.plot(test_dates, y_true, label="Actual Gold Price", color="blue")
    plt.plot(test_dates, y_pred, label="Predicted Gold Price (LSTM Tuned)", color="purple", linestyle="--")
